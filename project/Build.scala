...
object XScalaWTBuild extends Build {
    lazy val root = Project(
    	id = "ozb-xscalawt", 
    	base = file("."), 
    	settings = Defaults.defaultSettings /*++ Seq(updateTask) ++ InstallEclipseLibs.settings*/)
...
// 	import org.ozb.utils.ivy.IvyLocalRepositoryInstaller
// 	import org.ozb.utils.ivy.IvyLocalRepositoryInstaller.Input
...
// 	lazy val installEclipseJarsTask = installEclipseJars <<= InputTask(installEclipseJarsParser)(installEclipseJarsDef)
//     val installEclipseJarsDef = (parsedTask: TaskKey[Option[(String, Boolean)]]) => {
//     	(parsedTask, ivyJarPath, eclipseUpdateSite, eclipseLibs) map { case (opt, ivyJarPathOpt, eclipseUpdateSite, eclipseLibs) =>
//     		val withSources = opt.map(_._2).getOrElse(false)
...
// 				eclipseLibs map { lib =>
// 					val jarUrl = new java.net.URL(eclipseUpdateSite + "plugins/" + lib)
// 					val targetFile = dir / lib
// 					println(" downloading [%s] to [%s] ..." format (jarUrl, targetFile))
...
// 					}
// 					val input = Input(ivyJar, eclipseLib.org, eclipseLib.module, eclipseLib.revision, eclipseLib.jar.absolutePath, eclipseLib.srcJar)
// 					println("  input = " + input)
...
// 			}
//     		println("\ninstalled libraries:")
//     		installedLibs foreach (l => println("""%s"%s" %% "%s" %% "%s" %s """ format ("\t", l.org, l.module, l.revision, if (withSources) "withSources()" else "")))
//     	}
// 	} 
...
// 	/** adapted from sbteclipse (com/typesafe/sbteclipse/core/package.scala) */
...
// 	object EclipseLib {
// 		private val LibPattern = new scala.util.matching.Regex("""(.*)_.*""")
...
// 		private val RevPattern = new scala.util.matching.Regex("""_(\d+\.\d+\.\d+)\.""")
